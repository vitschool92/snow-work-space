...
-- T1. Full name with salary grade
SELECT EMPLOYEE_ID,
...
-- T2. Years of experience since hire date
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, HIRE_DATE,
       DATEDIFF(YEAR, HIRE_DATE, CURRENT_DATE()) AS YEARS_OF_EXPERIENCE
FROM EMPLOYEES
ORDER BY YEARS_OF_EXPERIENCE DESC;

-- T3. Salary as percentage of department total
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, E.LAST_NAME, E.DEPARTMENT_ID, E.SALARY,
       ...
       ROUND(E.SALARY / SUM(E.SALARY) OVER (PARTITION BY E.DEPARTMENT_ID) * 100, 2) AS SALARY_PCT
FROM EMPLOYEES E
ORDER BY E.DEPARTMENT_ID, SALARY_PCT DESC;

-- T4. Rank employees by salary within each department
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID, SALARY,
       RANK() OVER (PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) AS DEPT_SALARY_RANK
FROM EMPLOYEES;

-- T5. Mask email (show only first 3 chars)
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME,
       LEFT(EMAIL, 3) || '***' || SUBSTR(EMAIL, POSITION('@' IN EMAIL)) AS MASKED_EMAIL
FROM EMPLOYEES;

-- T6. Extract phone area code
SELECT EMPLOYEE_ID, FIRST_NAME, PHONE_NUMBER,
       SPLIT_PART(PHONE_NUMBER, '.', 1) AS AREA_CODE
FROM EMPLOYEES
WHERE PHONE_NUMBER IS NOT NULL;

-- T7. Flag employees above/below department average salary
...
-- T8. Cumulative salary by hire date
SELECT EMPLOYEE_ID, FIRST_NAME, HIRE_DATE, SALARY,
       SUM(SALARY) OVER (ORDER BY HIRE_DATE) AS CUMULATIVE_SALARY
FROM EMPLOYEES;

-- T9. Manager name lookup (self-join)
SELECT E.EMPLOYEE_ID, E.FIRST_NAME || ' ' || E.LAST_NAME AS EMPLOYEE_NAME,
       M.FIRST_NAME || ' ' || M.LAST_NAME AS MANAGER_NAME
FROM EMPLOYEES E
LEFT JOIN EMPLOYEES M ON E.MANAGER_ID = M.EMPLOYEE_ID;

-- T10. Department summary with headcount, avg/min/max salary
...
ORDER BY HEADCOUNT DESC;
 ...
 CREATE OR REPLACE API INTEGRATION "SNOW-WORK-SPACE"
